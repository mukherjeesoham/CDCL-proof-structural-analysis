...
# SM: Adding function to store the modularity score for each subgraph in a dictionary, and then to a file. 
def compute_hierarchical_community_structure_and_modularity(g, hierarchical_tree, current_node, path, output_directory, modularitydict):
	#calculates community structure
	#vertex_clustering = g.community_multilevel()
	vertex_clustering = g.community_edge_betweenness().as_clustering()
	plot_community_structure(vertex_clustering, path, output_directory)

        # Store the modularity in a dictionary
        # We compute the modularity using the clusters generated by our 
        # community detection algorithm.
        modularitydict[path] = igraph.Graph.modularity(g, vertex_clustering) 
